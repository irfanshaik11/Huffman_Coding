...
#include "./headers/dir_reader.h"
#include "./headers/wordfreq.h"
#include "./headers/inorder.h"
...
int encodeFile(char *filename, int firstcall){
    char nonterminal[50] = "!NONTERMINAL!";
    Init();
    int i = 0;
    char word[50];
...
int buildCodebook(char *filename, int recursive ){
    if (recursive == 0){
        printf("\n      CREATING HUFFMANCODEBOOK FOR ALL FILES IN GIVEN DIRECTORY\n");
    }else{
        printf("\n      CREATING HUFFMANCODEBOOK FILE FOR: %s \n", filename);
    }
...
//    Init();
    ...
    if (recursive != 0){
        Init();
        wordfreq(filename, -1); // builds the minheap
    }
...
//                printf("Both nodes are null error");
                return -1;
...
int decodeFile(char *filename){
    ...
    for(i=eof-4; i < eof; i++){
        if(fileext == 0){
            if(outfile[i] != '.'){
                printf("\n      FILE EXTENSTION IS NOT .HCZ FOR FILE: %s", filename);
                return -1;
            }
        }else if (fileext == 1){
            if(outfile[i] != 'h'){
            ...
            if (outfile[i] != 'z'){
                printf("\n      FILE EXTENTENSTION IS NOT .HCZ FOR FILE: %s", filename);
                return -1;
            }
        }
...
//                    printf("778\n");
                    ...
                    while(ptr!=NULL){
                        ...
                        encodeFile(filepath, firstcall);
                        
                        
                        ptr = strtok(NULL, delim);
                    }
                    
                    firstcall++; // avoids firstcall == 0 on second call
                }
//            printf("line 788\n");
...
//            fprintf(fptr,"Word | Frequency | Code\n");
//            fclose(fptr);
            char dir_reader_text[100] = "dir_reader_filepaths.txt";
            
            Init();
            wordfreq(dir_reader_text, 0);
